...
class FontsamplerHelpers {
    ...
    public function __construct($fontsampler) {
     ...
     */
    public function get_checkbox_features() {
     ...
     */
    public function get_css_file($id = false, $css = false) {
        if (false === $id) {
    ...
    public function write_custom_css_for_set($set) {
     ...
     * @return bool
     */
    public function write_custom_css($path, $css = null, $id = null) {
     ...
     */
    public function write_css_from_settings($settings) {
     ...
     * @return bool
     */
    public function write_less($input, $output, $vars, $is_path = true) {
     ...
     * @return bool|string
     */
    public function get_custom_css($set) {
        if (false === $set) {
            return false;
        }
     ...
     * @return bool
     */
    public function check_is_writeable($handle) {
     ...
     */
    public function fontfiles_json($font) {
        if (empty($font)) {
            return false;
        }
        $fonts_object = '{';
    ...
    public function is_legacy_format($format) {
        return in_array($format, $this->fontsampler->font_formats_legacy);
    }

    public function get_best_file_from_fonts($fonts) {
        $fontsFiltered = array();
        if (!$fonts) {
            return false;
        }
     ...
     */
    public function check_and_create_folders() {
     ...
     */
    public function settings_array_css_to_less($row) {
        $vars = array();
        if (is_array($row)) {
    ...
    // Replace $order["sampletexts"] with the ID that the JS frontend
    // needs to get the object and replace into the JS order
    public function replace_custom_elements($items, $r) {
        $return = array();
        $elements = array('sampletexts'); // TODO add others?
        foreach ($items as $item) {
            if (is_array($item)) {
                $return[] = $this->replace_custom_elements($item, $r);
    ...
    public function hide_changelog() {
